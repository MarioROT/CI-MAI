...
class EvolutionStrategy(object):
    ...
    def get_best_weights(self,):
        return max(self.rewards_history.items(), key=lambda x: x[1][0])[1][1]
    ...
    def run(self, iterations, print_step=10):
        ...
        for iteration in range(iterations):
            ...
            it_reg = self.get_reward(self.weights)
            self.rewards_history[iteration] = [it_reg, self.weights]
            
            if (iteration + 1) % print_step == 0:
                print('iter %d. reward: %f' % (iteration + 1, it_reg))
        if pool is not None:
            pool.close()
            pool.join()
        return [i[0] for i in self.rewards_history.values()]